function Output = LowerLayerForArea2(lambda_G_s, lambda_G_p, gamma)
...
ppv= [zeros(1,6), max(ppv)*cos(indexforppv(7:18)/9-14/9)+...
    0.15*max(ppv)*(rand(size(indexforppv(7:18)))-1), zeros(1,6)]/2;   
...
% if result.problem==0
%     disp('Problem2 lower level solve success')
% else
%     disp('Problem2 error')
% end
